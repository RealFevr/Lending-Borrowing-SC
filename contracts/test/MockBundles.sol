...
contract MockBundles is ERC721, Ownable, BundlesInterface {
    ...
    function depositNFTs(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                IERC721(nftAddresses[i]).ownerOf(ids[i]) == msg.sender,
    ...
    function withdrawNFTs(uint256 bundleLpId) external {
        require(
            IERC721(address(this)).ownerOf(bundleLpId) == msg.sender,
            "Not owner"
        );
        IERC721(address(this)).transferFrom(msg.sender, dead, bundleLpId);
        for (
            uint256 i = 0;
            i < bundleLpIdToBundle[bundleLpId].ids.length;
            i++
        ) {
            IERC721(bundleLpIdToBundle[bundleLpId].nftAddresses[i])
                .transferFrom(
                    address(this),
                    msg.sender,
                    bundleLpIdToBundle[bundleLpId].ids[i]
                );
        }
    }
    ...
    {
        return (
            bundleLpIdToBundle[bundleId].bundleLpId,
            bundleLpIdToBundle[bundleId].name,
            bundleLpIdToBundle[bundleId].nftAddresses,
            bundleLpIdToBundle[bundleId].ids
        );
    }
}
